...
export class Billing extends Component {
  ...
  update = async () => {
    ...
    return this.setState({ count: this.state.count + 1, formdata: [] });
  };
  ...
  getcount = async () => {
    let b = await Count(this.state.token);
    // console.log(b.data);
    // this.setState({ count: b.data });
  };
  ...
  render() {
    let { stockdata, User, text, daily, formdata, count } = this.state;
    ...
    return (
      <div>
        ...
        <div className="bill table mx-auto mp-5 center floated container">
          <div>
            <h5 className="text-center">start billing</h5>
          </div>
          ...
          <div>
            ...
            {formdata[0] && (
              <div>
                ...
                <button onClick={this.update}>Bill it</button>
              </div>
            )}
          </div>
        </div>
...
async function Count(token) {
  const res = await Axios({
    method: "get",
    url: "http://localhost:8000/bill/count",
    headers: {
      Authorization: `bearer ${token}`,
      "Content-Type": "application/json",
    },
